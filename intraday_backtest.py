...
from intraday_signals import evaluate, check_exit
...
def fetch_all_timeframes(client, symbol):
    bars_1h = client.get_bars(symbol, timeframe='1Hour', limit=10000)
...
def simulate_symbol(symbol, bars_1h, bars_15m, bars_5m, stop_pct, target_r=TARGET_R):
    ...
    for i, bar in enumerate(bars_5m):
        ...
        as_of_1h = _as_of_window(ts_1h, closes_1h, timedelta(hours=1), t)
        as_of_15m = _as_of_window(ts_15m, closes_15m, timedelta(minutes=15), t)
        if len(as_of_1h) < MIN_WARMUP_BARS or len(as_of_15m) < MIN_WARMUP_BARS:
            continue

        signal = evaluate(symbol, as_of_1h, as_of_15m, window_5m)
...
def main():
    ...
    print(f"Fetching 1h/15m/5m bars for {len(watchlist)} symbols...")

    per_symbol_bars = {}
    for symbol in watchlist:
        try:
            per_symbol_bars[symbol] = fetch_all_timeframes(client, symbol)
            b1h, b15m, b5m = per_symbol_bars[symbol]
            print(f"  {symbol}: {len(b1h)} 1h bars, {len(b15m)} 15m bars, {len(b5m)} 5m bars")
    ...
    for stop_pct in STOP_PCT_CANDIDATES:
        all_trades = []
        for symbol, (b1h, b15m, b5m) in per_symbol_bars.items():
            all_trades += simulate_symbol(symbol, b1h, b15m, b5m, stop_pct)
    ...
    all_trades = []
    for symbol, (b1h, b15m, b5m) in per_symbol_bars.items():
        all_trades += simulate_symbol(symbol, b1h, b15m, b5m, best_stop_pct)
    ...
    print("\n=== Verdict ===")
    if expectancy(holdout) > 0 and len(holdout) >= 5:
        print("Holdout expectancy is positive -- worth proceeding to Phase 3 (live session runner).")
    else:
        print("Holdout expectancy is NOT positive (or too few trades to trust) -- "
              "do not proceed to live capability with this rule set as-is.")
