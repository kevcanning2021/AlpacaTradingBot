"""Regression tests for the Phase 1 research-agent veto wiring in
trader.py: scan_and_execute() -- see the approved plan at
C:\\Users\\kevca\\.claude\\plans\\harmonic-crafting-donut.md. Mocked
throughout: a fake AlpacaClient (never a real one, per LESSONS.md #7 --
"unit testing a method with live side effects against the real client is
not a dry run") and a mocked agents.research_agent.propose (no real
Anthropic calls).
...
construction calls AlpacaClient()/TelegramNotifier() and loads/can write six
real state files from the repo root (peak_prices_state.json etc.), which a
unit test must never touch. Test signals are all crypto (BTC/USD): scanner.py
never sets a 'method' tag on crypto signals and scan_and_execute()'s drought
tracking is stock-only (`if not is_crypto:`), so a crypto buy's happy path
performs zero file writes -- no save-method mocking needed either.
...
class ScanAndExecuteAgentWiringTests(unittest.TestCase):
    ...
    def test_veto_disabled_by_default_agent_never_called(self):
        ...
        with patch.object(trader.OpportunityScanner, 'scan', return_value=[BUY_SIGNAL]), \
             patch('agents.research_agent.propose') as mock_propose:
    ...
    def test_veto_enabled_and_vetoed_blocks_the_buy(self):
        ...
        tm = make_manager(client)
        with patch.object(trader.OpportunityScanner, 'scan', return_value=[BUY_SIGNAL]), \
             patch('agents.research_agent.propose', return_value={'veto': True, 'confidence': 0.9, 'reasoning': 'earnings miss', 'risk_flags': [], 'failed': False}) as mock_propose:
            result = tm.scan_and_execute(watchlist=['BTC/USD'], position_size_usd=500, max_positions=2)
    ...
    def test_veto_enabled_and_not_vetoed_allows_the_buy(self):
        ...
        tm = make_manager(client)
        with patch.object(trader.OpportunityScanner, 'scan', return_value=[BUY_SIGNAL]), \
             patch('agents.research_agent.propose', return_value={'veto': False, 'confidence': 0.7, 'reasoning': 'no adverse news', 'risk_flags': [], 'failed': False}) as mock_propose:
            result = tm.scan_and_execute(watchlist=['BTC/USD'], position_size_usd=500, max_positions=2)
    ...
    def test_veto_enabled_and_agent_fails_open_allows_the_buy(self):
        ...
        tm = make_manager(client)
        with patch.object(trader.OpportunityScanner, 'scan', return_value=[BUY_SIGNAL]), \
             patch('agents.research_agent.propose', return_value={'veto': False, 'confidence': None, 'reasoning': 'agent call failed: timeout', 'risk_flags': [], 'failed': True}):
            result = tm.scan_and_execute(watchlist=['BTC/USD'], position_size_usd=500, max_positions=2)
        self.assertTrue(client.create_order.called)
        self.assertEqual(len(result['executed']), 1)
